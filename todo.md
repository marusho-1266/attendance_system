...
- [~] **test-enhancement**: テスト品質向上
  - 各テストファイル（Test.gs, ConstantsTest.gs, CommonUtilsTest.gs, SpreadsheetManagerTest.gs, BusinessLogicTest.gs, AuthenticationTest.gs, WebAppTest.gs, TriggersTest.gs, FormManagerTest.gs, MailManagerTest.gs）のテスト名・初期化・メッセージ・パフォーマンスを点検
  - テスト名は全体的に一貫性・分かりやすさ良好
  - 失敗時メッセージも明確な日本語で記述されており、現状問題なし
  - パフォーマンスも現状問題なし（今後テスト数増加時は実行時間計測を推奨）
  - MailManagerTest.gsはsetup/teardown実装済み。他ファイルも副作用が増えた場合は導入推奨
  - 今後の拡張時は副作用・実行時間・テストデータ干渉に注意
  - 🟡【課題】
    1. setup/teardown（初期化・後始末）共通化の徹底
    2. テストデータの完全分離
    3. パフォーマンステストの自動化・閾値アラート
    4. テストカバレッジの自動可視化・未テスト警告
    5. テスト失敗時の詳細ログ・デバッグ支援

- [~] **integration-test**: 統合テストの TDD実装
  - システム全体のE2E統合テスト設計・Greenフェーズ（最小実装でテストを通す）完了
  - runFullIntegrationTestで主要シナリオ（フォーム連携、認証、異常系、データ整合性）のGreenフェーズ（実際にテストが通る形）を実装
  - 既存runSystemIntegrationTest（初期セットアップ・基盤健全性検証）も維持
  - 次はRefactorフェーズ（テストデータ・モック整理、テストコードのリファクタ）に進む
...
- [x] 未退勤者リスト生成時の出勤・終業時刻の動的取得・冗長プロパティ削除 🟡 重要
  - Triggers.gsの未退勤者抽出ロジックで、emp.clockInTimeを固定値'09:00'から実際の出勤時刻取得関数に置換
  - emp.currentTimeを固定値'18:30'から標準終業時刻または現在時刻の動的取得に置換
  - emp.nameの冗長な再代入を削除（employeeNameで統一）
  - 上記のための「指定日・従業員IDの出勤時刻取得」関数を新規実装（Log_Rawシート検索）
  - 標準終業時刻取得はMaster_Employee参照、なければ現在時刻を返す
  - 既存の定数・共通関数を活用し、セル位置ハードコーディング禁止
  - テスト・既存機能への影響を確認
  - 見積時間: 1.5時間 